...
lower_green = np.array([40, 50, 50])  # Adjust these values for your specific green range
upper_green = np.array([80, 255, 255])  # Adjust these values for your specific green range
...
while True:
    ...
    if ret:
        ...
        # Bitwise-AND the original frame with the inverse of the green mask
        processed_frame = cv2.bitwise_and(frame, frame, mask=cv2.bitwise_not(green_mask))

        # Replace green pixels with the corresponding pixels from the pink image
        processed_frame[green_mask != 0] = pink_image[green_mask != 0]
